...
description: 'Eclipse 上で Vim ライクな操作を可能にするプラグイン Vrapper を使ったはなし'
pubDate: 2023-04-20
---
...
以下、Deepl先生の翻訳です。
> CDT、JDT、PyDevプラグインは、CDTパッケージ（C++開発ツール）、JDTパッケージ（Java開発ツール）、PyDevパッケージ（Python開発ツール）にのみ含まれるEclipseの機能にアクセスします。EclipseでC++の開発を行う場合、CDTがすでにインストールされている可能性があります。同様に、Java開発用のJDTもあります。*Vrapper CDTプラグイン*は*CDTパッケージ*に、*Vrapper JDTプラグイン*は*JDTパッケージ*に、*Vrapper PyDevプラグイン*は*PyDevパッケージ*に依存します。
2. 必要な機能を選択し、 「次へ」 を選択、インストール完了後に Eclipse を再起動すると導入が完了します。
